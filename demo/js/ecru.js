...
function getSummary(summary) {
	var sum = "<div class='summary'><h4>";
	var to = summary.start + summary.rows;
	if (summary.numFound == 0) {
		sum = sum + "No results Found</h4></div>";
		return $(sum);
	}
	...
	sum = sum + (summary.start + 1) + " -- " + to + " of " +  summary.numFound + " found ";
	if (backUrl) {
		sum = sum + "<a href='#' class='button' id='back'>go back to original search</a>";
	}
	sum = sum + 	"</h4></div>";
	...
	var isPrev = false;
	if (summary.prevUrl) {
		$p.append("<a class='solr button' href='" + summary.prevUrl + "'> << Previous</a>");
		isPrev = true;
	}
	if (summary.nextUrl) {
		var className = ".offset-by-eleven"
		if (isPrev) {
			$n.addClass(".offset-by-nine")
		}
		else {
			$n.add
		}
		$n.append("<a class='solr button' href='" + summary.nextUrl +  "'> Next >></a>");
...
function doFaceting(faceting) {
	...
	var str = "<h4>Refine Your Results</h4>  <ul>";
		 for (var i=0; i< faceting.listFacets.length; i++) {
			 ...
			 str = str + "<li class='f_list'><div><h5>" + header + "</h5><ul>\n" ;
...
function doItem(item, allReadings) {
    ...
    var thisResult = item.index + ". <span class='title";
    if (item.required) {
    	thisResult = thisResult + " required";
    }
...
function doReading(item, allReadings) {
	...
	str = str + "<br/>This ";
	if (item.required) {
		str = str + " <span class='req_read'>required</span> ";
	}
	str = str + "reading is <span class=";
	if (item.status === "Available") {
		str = str + "'avail'>available</span>";		
	}
	else {
		str = str + "'process'>being processed</span>";
	}

	if (item.annotation) {
		str = str + " <blockquote class='annotation'>Instructor's Note:</br>" + item.annotation + "</blockquote>";
	}
...
function doCourse(item) {
	...
	if (item.url) {
		str = str + "<br/><a href='" + item.url + " 'target='isite'>Course Site</a>";
	}
	else {
		str = str + "<br/><span class='no'>No course site has been identified at this time</span>";
	}
	return str;
}